...
- Shell 新增默认关闭的 DSH P0：`agentBackends.dsh` 自 schema 12 起存在；当前 `AppSettings` schema 13 增加品牌默认文案迁移。专属 `dsh_manager` 将 `@deepseek-ai/dsh@0.1.0-rc.6` 安装到壳私有前缀；package version 与固定 `lib/bin.js` 均须 canonicalize 后仍位于当前私有安装根内，符号链接/junction 越界 fail closed，再由受控 Node 绝对路径直接执行已验证 canonical 入口。P0 单后端实例只信任当前持有的 Child/process group 与本次分配的精确 `http://127.0.0.1:<port>`；运行时 PID/端口/URL/状态不持久化。启动期每 250ms 验证 HTTP 状态及前 512KiB 内固定页面身份 `__DSH_BOOT__`，运行后每 5s 只做轻量状态复检，连续 3 次失败进入 `degraded` 并在恢复后回到 `running`；控制中心在启用期间每秒刷新投影，Grid 在 degraded 期间保留同一 owned loopback iframe，避免短暂健康抖动卸载会话页面。`dsh.log` 每次写入前执行 10MiB 有界轮转，单行限制 64KiB，stdout/stderr 写入串行且全链路脱敏。Grid 可创建多个共享该后端的 DSH pane，关闭任意 pane（包括最后一个）不触发停服；只有应用退出/更新退出或用户在控制中心显式关闭时才做有界整树停止。
