...
- Shell 新增默认关闭的 DSH P0：`agentBackends.dsh` 自 schema 12 起存在；当前 `AppSettings` schema 13 增加品牌默认文案迁移。专属 `dsh_manager` 将 `@deepseek-ai/dsh@0.1.0-rc.6` 安装到壳私有前缀；Node 必须通过固定 `node:util.parseEnv` 能力探针（当前 pin 的官方/实测最低边界为 20.12.0）。Windows 检测到官方 `npm.cmd` 时不把批处理包装器直接交给 `CreateProcess`，而是解析同一 Node 安装目录下的 `node_modules/npm/bin/npm-cli.js`，由已配对的绝对 `node.exe` 直接执行；原生 npm executable 与 macOS npm 保持 direct exec。package version 与固定 `lib/bin.js` 均须 canonicalize 后仍位于当前私有安装根内，符号链接/junction 越界 fail closed，再由受控 Node 绝对路径直接执行已验证 canonical 入口。P0 单后端实例只信任当前持有的 Child/process group 与本次分配的精确 `http://127.0.0.1:<port>`；运行时 PID/端口/URL/状态不持久化。启动期每 250ms 验证 HTTP 状态及前 512KiB 内固定页面身份 `__DSH_BOOT__`，运行后每 5s 只做轻量状态复检，连续 3 次失败进入 `degraded` 并在恢复后回到 `running`；非主动 child 退出收割为 `crashed / E-DSH-005` 并写入受管日志。控制中心在启用期间每秒刷新投影，Grid 在 degraded 期间保留同一 owned loopback iframe，避免短暂健康抖动卸载会话页面。`dsh.log` 每次写入前执行 10MiB 有界轮转，单行限制 64KiB，stdout/stderr 写入串行且全链路脱敏。Grid 可创建多个共享该后端的 DSH pane，关闭任意 pane（包括最后一个）不触发停服；只有应用退出/更新退出或用户在控制中心显式关闭时才做有界整树停止。
...
## Known Gaps
- DSH P0 自动化已覆盖 schema/argv/pin/URL 持久化/Unix descendant-kill；macOS 当前 pin 已完成私有安装、WKWebView 实际交互、应用/更新退出、故障注入和 ≥70min 后端共存证据，真实 pane 长时交互仍保留 release candidate 人工复核。Windows 首轮 G3 已证明 GUI 冷启动能找到 Node/npm，同时暴露 `npm.cmd` 被直接 CreateProcess 的安装阻断；代码已改为 paired `node.exe + npm-cli.js` 并通过跨平台单测/Windows 配置静态 gate，仍需用户用新 Windows 包重跑私有安装、WebView2 交互与 `taskkill /T` 整树证据，不得从 macOS 或静态检查外推。
