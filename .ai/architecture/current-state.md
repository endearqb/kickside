...
- Agent Room Admin API 已按 Accepted ADR 实现并由 `KIMI_AGENT_ROOM_ENABLED` 默认关闭：Agent/Room/Member CRUD、Message/Run Forward、Abort/Retry、Timeline、Pane Sync、Observation Pin/List、Event long poll、Capabilities 与 Bridge Status summary。新路由使用既有 Admin auth/envelope、strict 1 MiB body 和脱敏 typed error；React 不持有 token。`record_only` 保持纯持久化；其他策略只通过 ready Server Provider 执行。
