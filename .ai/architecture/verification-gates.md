...
- 常规 macOS 公开发布必须针对最终 DMG 内的 app 通过 `codesign --verify --deep --strict`、`spctl --assess`、notarization 与 stapling；Developer ID certificate、Apple ID app-specific password 与 Team ID 只存 Actions Secrets。`0.1.24`、`0.2.0` 与另经 accepted 决策明确批准的 `0.2.1` 临时未签名例外，改由 CI 验证 `.app` 仅含 ad-hoc signature 且无 Apple signing authority，并要求对应 Release 顶部标注未签名/未公证警告；例外不授权任何 Gatekeeper、双平台 G3 或生产就绪结论，后续版本不得自动沿用。
