...
3. DSH 以固定源码常量 pin。KickSide 自有支持矩阵为 Node 22.19+ 的 22.x 或 24+，并额外要求固定 `node:util.parseEnv` 能力探针；rc.6 npm 元数据未声明 `engines`，Node 20.12 真实可运行证据只作为观察性兼容，不取得产品支持权限。安装到壳私有前缀的临时目录，完成版本与入口校验后再替换；`package.json` 与入口 canonicalize 后必须仍在当前私有安装根内，符号链接/junction 越界时 fail closed；生产直接执行已验证 canonical 入口，不使用运行时 `npx` fallback。运行选择与安装工具链选择分离，安装只接受 Node/npm 同目录配对；Windows `npm.cmd` 优先解析同工具链 `npm-cli.js` 并由 Node 直接执行，缺失时只允许通过 canonical System32 `cmd.exe` 的 `/D /S /V:OFF /C` 回退执行固定包名、版本与 flags，动态路径只经引号包围的专用环境变量传入，其他命令处理器 fail closed。
