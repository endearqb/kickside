...
3. Tauri `productName` 在所有语言打包配置中统一为 `KickSide`，从而稳定 `.app`、DMG、NSIS 与 MSI 的公开产品名；中文界面仍显示 `KickSide 启伴`。由于 Tauri MSI UpgradeCode 默认由 `productName` 派生，Windows 配置必须显式保留改名前 `kimi sidekick` 的 UpgradeCode `dfa197f9-0e61-5393-a612-7e4ca38701cc`；NSIS 通过有界 preinstall hook 迁移旧产品名注册项。
...
- 应用数据沿用 `com.kimi.shell` 与原目录；Windows 安装身份另外由显式 MSI UpgradeCode 与 NSIS 旧品牌迁移 hook 维持，不能只依赖 bundle identifier。
...
- Rust fmt/test，确认 `identifier` 仍为 `com.kimi.shell`；bundle gate 固定旧 MSI UpgradeCode 并检查 NSIS 迁移 hook。
