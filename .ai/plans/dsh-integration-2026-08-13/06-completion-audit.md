...
| P0 代码 | 完成 |
| macOS arm64 G3 | 当前 pin 的安装、启动/readiness、WKWebView、TERM/残留、端口与崩溃故障注入已完成；Kimi + DSH 后端共存按产品负责人接受的 ≥70min 门槛运行 79m14s，零失败；隔离签名 0.2.0→0.2.1 updater 退出通过；真实 pane 长时交互仍由 release candidate 人工复核 |
| Windows x86_64 G3 | 首轮已识别 Node/npm，但暴露 `npm.cmd` 直接启动失败与旧 `kimi sidekick` 未提示两项阻断；代码已修，并显式保留改名前 MSI UpgradeCode，等待新包原生复测，仍阻止宣称“双平台可发布” |
...
| 需求 | 状态 | 代码/证据 |
|---|---|---|
| FR-1 环境检测 | 完成，Windows 安装复测待回填 | `nodejs_locator` 覆盖 GUI 冷环境常见 Node 管理器；固定 `util.parseEnv` 能力探针拒绝 Node 18/过旧运行时，提示最低 Node 20.12.0；Windows 首轮已证开始菜单冷启动可发现 Node/npm；官方 `npm.cmd` 现由 paired `node.exe` 直接执行同目录 `npm-cli.js`，避免批处理被误交给 CreateProcess；缺失或过旧均为 E-DSH-001 |
...
| Windows 旧品牌安装迁移 | 代码完成，真机待复测；NSIS 精确发现历史 NSIS/MSI 产品名，交互安装先提示，确认后保留数据卸载，passive/silent 自动迁移，失败 fail closed；MSI 显式沿用改名前 `kimi sidekick` UpgradeCode |
...
- Node 最低能力实测：Node 18.20.8 可完成 npm 安装但启动时因 DSH app-boot 导入不存在的 `node:util.parseEnv` 退出；Node 20.20.2 为 422ms ready / 754ms 软停，Node 22.23.2 为 1011ms ready / 252ms 软停，两者端口均释放且未强杀。Node 官方记录该 API 自 20.12.0/21.7.0 引入，因此 production preflight 同时给出 20.12.0 最低提示并实际探测该能力；Node 24.19.0 已有前述多轮/长稳证据。
...
- macOS Kimi + DSH 后端共存：Node 24.19.0 下持续 4754s（79m14s），每 5s 采样，共 950 次且 0 次失败；Kimi HTTP max/p95 9/2ms，DSH HTTP max/p95 15/2ms；结束时两端均软停、进程组消失、端口关闭，显式 PID 也已确认不存在。结果保存于 `/tmp/kickside-coexist-2h-result.json`；其中 `aborted=true` / `completed=false` 是因为文件仍记录原 7200s 目标、按产品负责人新接受的 70min 门槛主动停止，不是运行失败。
- macOS updater 真版本差：以临时 `KickSide Updater E2E` / `com.kimi.shell.updater-e2e`、一次性签名密钥和 loopback manifest 构造 0.2.0→0.2.1。首轮真实下载/验签/替换证明原实现会停止 Kimi/DSH 却不退出 App；补上安装成功后的显式 `app.exit(0)` 后复跑，约 1.25s 内 App PID 69928、DSH PID 69952、Kimi PID 70035 全部消失，34080/59661 无 listener，安装目录版本为 0.2.1。替换后的 0.2.1 再启动成功、控制中心显示 `v0.2.1 · 已是最新版本`，标准 Quit 后新一轮三个 PID/两个端口也清零。证据保存在 `/tmp/kickside-updater-e2e.wN5zfD`；一次性私钥已删除，正式 `com.kimi.shell` 配置与发布密钥均未触碰。
- 用户提供的 macOS 截图证明当前 pin 已在 KickSide WKWebView pane 内完成加载、会话交互、多 pane 并排和目录切换投影。
- 最新本地测试包：`KickSide_0.2.0_macos_arm64_dsh-p0-updater-exit.zip`，SHA-256 `ef8fd3a27f9b345260f88b272adb8cc5dd9205ea578b26bad2670f49955befb0`，20368390 bytes；包含此前 DSH recovery/Node 20.12/page identity/canonical 入口/E-DSH-005 能力及 updater 安装成功显式退出修复；从 zip 解包后 strict deep ad-hoc 签名、主程序/Bridge arm64、`com.kimi.shell`、`KickSide` 与 0.2.0 均复验通过。
...
2. Windows 原生 Kimi + DSH 长稳与故障注入发布矩阵；macOS 后端共存和 updater 真版本差退出已通过，但自动探针不证明真实 pane 的长时输入、会话切换与目录投影。
...
- 下一发布门：用户在 Windows 11 测试机运行 `07-windows-g3-checklist.md`；macOS release candidate 只需补真实 pane 长时交互，不再重复等待 2h 后端探针或 updater 真版本差退出。
