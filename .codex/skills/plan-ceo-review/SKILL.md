...
Analyze the plan. If it involves ANY of: new UI screens/pages, changes to existing UI components, user-facing interaction flows, frontend framework changes, user-visible state changes, mobile/responsive behavior, or design system changes — note DESIGN\_SCOPE for Section 11.
...
-   **STATUS**: "clean" if 0 unresolved decisions AND 0 critical gaps; otherwise "issues\_open"
    
-   **unresolved**: number from "Unresolved decisions" in the summary
    
-   **critical\_gaps**: number from "Failure modes: \_\_\_ CRITICAL GAPS" in the summary
    
-   **MODE**: the mode the user selected (SCOPE\_EXPANSION / SELECTIVE\_EXPANSION / HOLD\_SCOPE / SCOPE\_REDUCTION)
...
-   **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`gstack-config set skip\_eng\_review true\` (the "don't bother me" setting).
...
-   **CLEARED**: Eng Review has >= 1 entry within 7 days with status "clean" (or \`skip\_eng\_review\` is \`true\`)
    
-   **NOT CLEARED**: Eng Review missing, stale (>7 days), or has open issues
    
-   CEO, Design, and Codex reviews are shown for context but never block shipping
    
-   If \`skip\_eng\_review\` config is \`true\`, Eng Review shows "SKIPPED (global)" and verdict is CLEARED
...
-   For each review entry that has a \`commit\` field: compare it against the current HEAD. If different, count elapsed commits: \`git rev-list --count STORED\_COMMIT..HEAD\`. Display: "Note: {skill} review from {date} may be stale — {N} commits since review"
