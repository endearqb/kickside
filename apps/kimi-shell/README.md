...
- Workspace Grid V2 壳层：常驻 `Kimi Code Web` 与 `Kimi Chat`，支持 1/2/3/4/5/6 个可见窗格、最多 12 个总窗格与 Pane Shelf 收纳。旧 Agent Room Pane 在加载 state/saved layout 时被移除并修复布局引用；V2 兼容输入保留一个发布周期后退出。
...
- Node.js 22.19+（Kimi Code 应用内首次安装使用 npm；升级跟随当前命中的 Windows 原生、npm 或 pnpm 安装来源）
- pnpm 10.34.4
- Rust stable
