...
- Workspace Grid V2 壳层：常驻 `Kimi Code Web` 与 `Kimi Chat`，支持 1/2/3/4/5/6 个可见窗格、最多 12 个总窗格与 Pane Shelf 收纳。小助手设置中的 Agent Room 开关启用且 Workspace 存在 Kimi Code Pane 时，标题栏显示独立单例工具窗口入口；旧无 iframe 本地 Pane 在 7 天 Product Gate 前继续保留为回滚与能力兼容入口。V2 使用独立 state/saved-layout keys，从 V1 只读迁移并保留旧键回滚。
...
- 控制中心：小助手设置以 9 个互斥展开项承载小助手更新、安装/升级、右键菜单、API 配置、默认工作目录、Agent Room、外部 IM 通道、Kimi Doctor 和日志；Agent Room 默认关闭并持久化到应用设置，API 配置与微信/飞书扫码均在设置项内完成，复用外部 Server 时侧边栏底部提供重新连接
...
- `src-tauri/src/agent_room_event_pump.rs` 负责默认关闭的 Agent Room Event Pump：25 秒长轮询、单调 Cursor、generation 取消、退避、30 秒空闲 grace、sidecar 重启恢复，并只定向投递到 `main` / `agent-room`；`src/services/agentRoomService.ts` 是 React 唯一的 Agent Room command/event 薄入口。
- `src/features/agent-room/AgentRoomWindowApp.tsx` 是独立 Agent Room 窗口入口，承载房间切换/创建、执行成员、Message→Run 任务动态、Compact Composer、内嵌 one-shot 审批和精确 Session 打开；窗口不启动 `useShellController`，不读取主窗口 Store。`src/features/workspace-grid/usePaneSessionRegistry.ts` 在 capability 可用时持续发布 Code Pane 快照，独立窗口从 Bridge Observation 的只读 `panes` 投影识别当前/可见/收纳 Session。
...
- `src/features/workspace-grid/usePaneSessionRegistry.ts` 在应用内 Agent Room 开关生效时把 Code Pane 全量快照以 250ms trailing debounce 同步到 Bridge；`agentRoomObservationStore.ts` 只保存去重 Session 观察投影、pin、event seq 与能力/泵状态，不复制完整 Session。generation 冲突使用 Admin error details 恢复，普通断线有界退避。
...
- Agent Room Admin token 只存在于 Rust Bridge 生命周期状态；React 只能调用 Tauri allow-list 中的 `agent_room_*` commands 和监听脱敏事件，不得读取 locator、Runtime Client、HTTP Client 或 token。`agent-room` 窗口使用独立 `agent-room-command-access`，不具备 main 的 Agent/Connector、Pane sync、安装、Secrets、文件读写或 Webview 创建权限。
