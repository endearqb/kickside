...
## Skill Center Split + Bridge Multi Connector Shell Follow-up (2026-03-23)

### Plan

- [x] 将本轮实现拆成两段：先完成 Skill Center 双分区与工作区洞察，再补完 shell 侧 IM Bridge 多 connector 收口。
- [x] 为 Skill Center 新增工作区 profile 只读模型与 Tauri 命令，返回最近使用 / 上次 session 使用记录。
- [x] 将 Skill Center 控制中心卡片拆成“技能管理 / 工作区洞察”两个内部子分区，保留现有安装、导入、信任、应用链路。
- [x] 在工作区洞察中展示当前工作区路径、active session、当前工作区已应用技能、最近使用、上次 session 使用、用户全局已应用数量，并补齐空态与“查看技能 / 应用到当前工作区”交互。
- [x] 将 `kimi-shell` Rust / TypeScript / 前端 Bridge 模型从固定 `channels` 升级到 connector 读模型，补齐 `connectorId` / `connectorLabel` 的状态、binding、approval、session 展示。
- [x] 将 Bridge 设置与运行面板改成 connector 列表 + 详情的管理结构，并补齐 connector CRUD 与 secret 更新命令。
- [x] 运行 Go / Rust / 前端验证，若构建再次被文件锁阻塞则先定位并清理锁占用，再拿到干净通过。

### Validation

- [x] 运行 `cargo test --manifest-path apps/kimi-shell/src-tauri/Cargo.toml`，确认 Skill Center 新读模型与 shell connector 改造未引入 Rust 回归。
- [x] 运行 `pnpm -C apps/kimi-shell build`，确认 Skill Center UI 与 Bridge UI 改造可干净构建。
- [x] 运行 `go test ./...`（`apps/kimi-im-bridge`），确认 shell 侧 connector 收口未破坏 sidecar 既有契约。
- [ ] 手工检查 Skill Center 在“技能管理 / 工作区洞察”之间切换不丢状态，洞察区能查看当前工作区与恢复上次使用技能。
- [ ] 手工检查 Bridge connector 列表、详情、binding / approval / session 归属展示与 connector CRUD 基本交互。

### Retrospective

- [x] 记录这轮拆分里暴露出的 Skill Center 工作区模型边界、shell connector 兼容策略，以及哪些 legacy `channels` 入口还能删除。
- [x] Skill Center 的“工作区洞察”第一版适合严格停留在只读派生层：状态来自现有 installed/apply/profile 数据的拼装，而不是再引入一套统计写模型；这样能把 UI 拆分做出来，同时避免把 recent/last-session 语义扩散成新的持久化负担。
- [x] shell 侧多 connector 收口必须同时兼容两类 legacy 入口：一类是 Go admin payload 仍可能返回 `channels` 字段，另一类是本地 secrets/settings 里仍有顶层 `telegram` / `feishu` 结构；这轮先把它们都归一到 connector 读模型，再逐步删除写路径。
- [x] bridge 运行态、binding、approval、session 一旦进入多实例场景，展示归属必须依赖稳定 `connectorId`，`platform` 只能退回为分组和标签语义；后续可以继续清理仍以 `platform` 作为主查找键的前端 helper。

## Control Center UI Consolidation (2026-03-23)

### Plan

- [x] 移除控制中心头部单独的“控制中心”标题，仅保留 tabs 导航与关闭按钮。
- [x] 统一控制中心 tabs 使用大屏按钮样式，移除小屏下的二次缩小退化。
- [x] 在概览页“当前没有阻塞项”空状态下恢复 Agent 使用提示卡和刷新交互。
- [x] 将优先任务中的“查看最近日志”替换为技能中心入口，并展示技能相关状态。
- [x] 收口快速设置右侧重复标题：保留顶部当前步骤标题，下方详情卡改为通用操作型标题。
- [x] 运行前端构建验证，并补充本次 UI 收口回顾。

### Validation

- [x] 运行 `pnpm -s build`（`apps/kimi-shell`），确认控制中心视图和样式改动未引入构建错误。
- [ ] 手工检查控制中心 fullscreen / modal 头部、概览提示卡、技能中心入口与快速设置标题层级。

### Retrospective

- [x] 记录这轮 UI 收口暴露出的控制中心头部、空状态卡片和步骤详情层级约束。
- [x] 控制中心头部在高密度桌面场景里应优先承担“导航切换”而不是重复页面命名；同一层同时放标题和 tabs，信息价值通常低于占用的横向空间。
- [x] 概览空状态不该退化成纯结论文案；当没有阻塞项时，给用户一条可刷新的 Agent 协作提示，比重复说“系统正常”更有后续动作价值。
- [x] 快速设置右侧需要明确“页面级当前步骤”与“卡片级操作详情”的层级分工；两层都用同一个步骤名做主标题时，视觉上会像重复渲染而不是强化信息。

## WeChat Agent SDK Bridge Research (2026-03-23)

### Plan

- [x] 梳理当前 `apps/kimi-im-bridge` 的 adapter、binding、approval、delivery 与 connector 扩展边界。
- [x] 调研 `aihubmix/wechat-agent-sdk` 的接入方式、消息模型、运行前提与对 Agent 的封装方式。
- [x] 对照本项目 `im bridge` 与微信 SDK 的模型差异，判断“能否桥接、以什么层级桥接、主要风险是什么”。
- [x] 输出研究结论、建议接入路径与不建议直接推进的条件。

### Validation

- [x] 结论同时引用了本地仓库事实和外部 SDK 事实，不只停留在理论猜测。
- [x] 已明确区分“理论可行”与“并非低成本直插”的判断：当前更像新增一个微信 connector / adapter，而不是现成桥接。

### Retrospective

- [x] 记录本次研究里暴露出的 bridge 平台适配抽象边界和未来新增 connector 的判断标准。
- [x] 当前 IM bridge 的中台能力已经足够复用，但平台接入层仍是“硬编码平台 + 独立 adapter 实现”，因此新增微信的关键不是 runtime，而是补齐微信侧的入站、回复、审批交互与 connector 配置抽象。
- [x] `wechat-agent-sdk` 更接近“自带消息循环和 Agent 适配的一体化 Python 微信 bot 框架”，不是现成的 Go adapter 库；若直接接入本项目，更适合作为外部前门或参考实现，而不是原样嵌进现有 sidecar。
