...
- 验证结果：
  - `pnpm -C apps/kimi-shell tauri build` 通过
- MSI: `apps/kimi-shell/src-tauri/target/release/bundle/msi/Kimi Desktop Shell_0.0.12_x64_en-US.msi`，`6782976` bytes，`2026-03-07 11:43:01`
- NSIS: `apps/kimi-shell/src-tauri/target/release/bundle/nsis/Kimi Desktop Shell_0.0.12_x64-setup.exe`，`5142243` bytes，`2026-03-07 11:43:08`
- 剩余说明：
...
- Verification:
  ...
  - New artifacts:
- `apps/kimi-shell/src-tauri/target/release/bundle/msi/Kimi Desktop Shell_0.0.12_x64_en-US.msi`
- `apps/kimi-shell/src-tauri/target/release/bundle/nsis/Kimi Desktop Shell_0.0.12_x64-setup.exe`
- Remaining note:
...
- `cargo check --manifest-path apps/kimi-shell/src-tauri/Cargo.toml` passed.
- Generated artifacts:
- `apps/kimi-shell/src-tauri/target/release/bundle/msi/Kimi Desktop Shell_0.0.14_x64_en-US.msi` (`6795264` bytes, `2026-03-08 12:24:04`)
- `apps/kimi-shell/src-tauri/target/release/bundle/nsis/Kimi Desktop Shell_0.0.14_x64-setup.exe` (`5148649` bytes, `2026-03-08 12:24:13`)
...
## Current Plan (Kimi 安装弹窗与应用内安装控制台改造实施)

### Checklist

- [x] 引入统一安装任务目录、安装会话快照与 Channel 流式输出模型
- [x] 实现 Rust 侧 `InstallManager`、应用内 PowerShell 托管执行与外部管理员兜底
- [x] 新增安装相关 Tauri 命令并保留旧安装命令兼容包装
- [x] 改造前端 `useShellController`，切换到安装快照 + Channel 模型
- [x] 用统一 `InstallFlowModal` 替换现有完整命令弹窗，并简化控制中心安装入口
- [x] 运行 `cargo check --manifest-path apps/kimi-shell/src-tauri/Cargo.toml`
- [x] 运行 `pnpm -C apps/kimi-shell build`
- [x] 回填本节“Review”与验证结果
...
- [x] 核心安装链路不再依赖外部 PowerShell 窗口才能看到执行过程
- [x] `quick_install_core / install_uv / install_python313 / install_kimi / upgrade_kimi` 可在应用内显示实时日志
- [x] `install_git / install_nodejs` 明确标记为可选增强项，并走外部管理员兜底
- [x] 复制命令与实际执行步骤来自同一份任务定义
- [x] 关闭再打开安装弹窗后仍能看到最近一次安装会话结果
- [x] `Git` 未安装不会阻塞 `coreReady`
- [x] `cargo check --manifest-path apps/kimi-shell/src-tauri/Cargo.toml` 通过
- [x] `pnpm -C apps/kimi-shell build` 通过
...
- Actual changes:
- 新增 `apps/kimi-shell/src-tauri/src/install_manager.rs`
  - 引入统一任务目录、`InstallManager` managed state、安装快照、Channel 推送与日志缓冲。
  - `quick_install_core / install_uv / install_python313 / install_kimi / upgrade_kimi` 走应用内 PowerShell 管道输出。
  - `install_git / install_nodejs` 改为外部管理员 PowerShell 兜底。
- 更新 `apps/kimi-shell/src-tauri/src/lib.rs`
  - 注册 `InstallManager` state。
  - 新增 `register_install_session_channel / get_install_flow_catalog / get_install_session_snapshot / start_install_task / cancel_install_task`。
  - 旧命令 `install_kimi_dependencies / install_kimi_cli / upgrade_kimi_cli / install_nodejs` 改为兼容包装。
- 更新 `apps/kimi-shell/src-tauri/src/types.rs`
  - 新增安装任务、会话、日志、事件类型。
  - 扩展 `InstallProbeStatus`，增加 `wingetReady` 与 `coreReady`。
- 更新前端 `apps/kimi-shell/src/app/useShellController.ts`
  - 注册 Tauri `Channel`，消费安装快照与日志。
  - 新增安装任务启动、取消、快照恢复与目录拉取逻辑。
- 新增 `apps/kimi-shell/src/features/control-center/InstallFlowModal.tsx`
  - 集成环境状态、一键基础安装、升级/可选增强项、控制台日志与复制动作。
- 更新 `apps/kimi-shell/src/features/control-center/ControlCenterView.tsx`、`apps/kimi-shell/src/App.tsx`、`apps/kimi-shell/src/App.css`
  - 将安装入口切到统一安装弹窗，并补充最小样式。
- Verification:
- `cargo check --manifest-path apps/kimi-shell/src-tauri/Cargo.toml` 通过。
- `pnpm -C apps/kimi-shell build` 通过。
- 静态实现上已满足：
  - 基础安装任务在应用内流式显示日志。
  - Git / Node.js 走外部管理员兜底。
  - 任务定义同时驱动复制命令与执行步骤。
  - 安装弹窗重新打开时会通过快照恢复最近一次会话。
- Remaining note:
- `backend_manager.rs` 里旧安装链路与兼容 catalog 仍保留，当前会产生一组 `dead_code` warning；它们未影响构建通过，后续可单独清理。
- Follow-up completion:
  ...
  - `apps/kimi-shell/src/features/control-center/ControlCenterView.tsx`
    - Simplified the onboarding install card to environment summary + `打开安装与升级` + `重新检测`, while keeping manual path confirmation available.
  - `apps/kimi-shell/src\App.css`
...
- Actual changes:
- Updated [install_manager.rs](../../apps/kimi-shell/src-tauri/src/install_manager.rs) so the shared PowerShell bootstrap now appends `uv tool dir --bin` to `PATH` when `uv` is available.
